...
class ReviewKD(nn.Module):
    def __init__(
        ...
        for idx, in_channel in enumerate(out_channels):
            reabfs.append(ABF(in_channel, mid_channel, out_channels[idx], idx > 0))
        self.reabfs = reabfs
        self.to('cuda')
    ...
    def forward(self, x):
        ...
        for features, abf, shape, out_shape in zip(results[1:], self.reabfs[1:], shapes[1:], out_shapes[1:]):
            out_features, res_features = abf(features, res_features, shape, out_shape)
            result.append(out_features)
...
from Fuseblock import LayerNorm
